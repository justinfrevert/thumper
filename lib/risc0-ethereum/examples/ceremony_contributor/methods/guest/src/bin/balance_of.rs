...
// TODO: verify receipt inside

fn main() {
    // Read the input from the guest environment.
    let input: EthViewCallInput = env::read();
    let private_key: Address = env::read();
    let contribution_bytes: Vec<u8> = env::read();

    let current_contribution = G1Projective::deserialize_compressed(&*contribution_bytes).unwrap();

    // let current_ceremony_index: u8 = env::read();
    ...
    // let compressed_contribution: Vec<u8> = receipt.journal.decode().unwrap();
    ...
    // Prove we have the key for the participant who's turn it is baased on their position in the participants list
    // assert!(&returns._0[current_ceremony_index] == digest.as_words());
    ...
    // env::commit(&(compressed_bytes, current_contribution_index + 1));
